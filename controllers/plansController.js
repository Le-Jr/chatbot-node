...
const client = new MercadoPagoConfig({
  accessToken:
    "TEST-1518391375042209-031618-771d1f999699cb8e15b268d22961eff1-2329930160",
});
...
export class plansController {
      ...
      plans.map(async (plan) => {
        await preference
          .create({
            body: {
              back_urls: {
                success: `https://8b61-189-90-112-73.ngrok-free.app/user/${user.id}/${user.token}`,
              },
              items: [
                {
                  title: plan.name,
  ...
  static async hasActivePlan(req, res, next) {
    ...
    const hasActivePlan = await ClientsPlans.findOne({
  ...
  static async PaymentWebhookHandler(req, res) {
    await fetch(`https://api.mercadopago.com/v1/payments/${req.body.data.id}`, {
      ...
      headers: {
        ...
        Authorization:
          "Bearer TEST-1518391375042209-031618-771d1f999699cb8e15b268d22961eff1-2329930160",
      },
    })
      .then((response) => {
      ...
      .then(async (res) => {
        await ClientsPlans.create({
          serviceId: res.metadata.plan_id,
          clientId: res.metadata.user_id,
        });
        await Clients.update(
          { wasPurchased: 1 },
          { where: { id: res.metadata.user_id } }
        );
      });
